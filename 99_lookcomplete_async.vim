...
function! s:on_text_changed_i() abort
    ...
    call Log('check_changes, previous_position:' . string(l:previous_position[1]) . ' current_position:' . string(getcurpos()[1]))
...
function! s:async_get_source(ctx) abort
    let s:callbacks = {
...
function! s:update_pum(ctx, words) abort
    let l:old_ctx = a:ctx
